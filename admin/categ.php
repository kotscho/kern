<?php
/*
 * kdos: add the unit_id which is basically something like the "menu_id.
 * You will have to inject most of the urls with this value
 *
 * 20122011 Fix categselector to support unit id
 */
error_reporting(0);
require_once('functions/adminFunctions.inc.php');
require_once(CLASS_PATH.SYS_FOLDER."class.application_nodes.php");
require_once(CLASS_PATH.SYS_FOLDER."class.application_nodes_types.php");
require_once(CLASS_PATH.SYS_FOLDER."class.application_nodes_stages.php");
require_once(CLASS_PATH."class.content_category.php");
require("xajax.common.php");
is_alive();

$ob = new application_nodes($dbObj);

$obs = new application_nodes_stages();
$unit = (isset($_GET['unit_id'])?$_GET['unit_id']:$_GET['new_unit_id']);
switch($_GET['action']){
	case 'edit': 
		$current_id = explode('_', $_GET['id']);
        header('location: categ.php?edit=true&id='.$current_id[1].'&unit_id='.$unit);
		break;
	case 'delete':
        if(empty($isReadonly))
            $ob->deleteNode($_GET['id'], $unit);
		break;
    case 'choose':
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#black").treeview({
			...
			url: "source.php?unit=<?php print $unit; ?>"
		});
	...
	function triggerAction(action, id, txt){
		...
		location.href = 'categ.php?action='+action+'&id='+id+'&unit_id='+<?php print $unit; ?>;
	}
	</script>
<script type="text/javascript">
	$(function() {
...
<div class="content">
    <h5>Διαχείρηση Κατηγοριών (<?php print application_nodes_helper::getCategUnitById($unit); ?>)</h5>
<div class="sublinks">
<br />
<a href="categ.php?add=true&amp;unit_id=<?php print $unit;?>" title="Εισαγωγή νέας κατηγορίας">Εισαγωγή νέας κατηγορίας</a>
<?php
if(isset($_GET['add']) || isset($_GET['edit']) || isset($_GET['del'])):

?>
&nbsp;|&nbsp;<a href="categ.php?unit_id=<?php print $unit;?>" title="Επιστροφή">Επιστροφή</a>
<?php
endif;
?>
<br /><br />
...
?>
<form method="post"  name="categ_form" id="categ_form" class="mainform" action="<?php print  $_SERVER['PHP_SELF'];?>">
  <fieldset class="userfieldset">
   ...
   onclick="xajax___setTempSession(document.categ_form.name.value, document.categ_form.description.value, document.categ_form.datepicker.value, document.categ_form.datepicker_to.value); pop(<?php if(!empty($ob->ID) ):?>'categselector.php?nodeid=<?php print  (isset($_GET['nodeid'])?$_GET['nodeid']:$_GET['id']); ?>'); 
    <?php 
      else: print "'categselector.php');";
      endif;
    ?>">
   αλλαγή</a>&nbsp;η&nbsp;<a href="javascript: void();"  onclick="document.getElementById('parenttype').value = 'no parent'; document.getElementById('passed_parentid').value = 0;">"no parent" </a>
    ...
    <input type="button" <?php print  (!empty($isReadonly))?'disabled="disabled"':''; ?> name="submitbutton" class="input_button" onclick="javascript: validator('categ_form');" value="Εισαγωγή" />
   </fieldset>
</form>
<?php
endif;
